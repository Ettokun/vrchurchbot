    ...
    function(accessToken, refreshToken, profile, done) {
      process.nextTick(function() {
        ...
        let loginembed = new Discord.MessageEmbed()
          ...
          .setTimestamp();
        client.channels.cache.get(config.logs).send(loginembed);
      });
    }
  )
);
...
app.post("/giveawaysubmit", async (req, res) => {
  ...
  let vrchurch = client.guilds.cache.get(config.server);
  let shadow = client.users.cache
    .get(config.ownerID)
    .send(
...
client.on("message", async message => {
  ...
  if(command === "poll"){
    ...
    let m = await message.channel.send(args.slice(1).join(" "))
  ...
  if (command === "say") {
    ...
    let bs = message.guild.roles.cache.find(r => r.name === "BotSpeak");
    if(message.member.roles.cache.find(r => r.name === "BotSpeak")) return;
  ...
  if (command === "forceping") {
    let roleName = message.content
      ...
      .slice(1)
      .join(" ");
    let pingrole = message.guild.roles.cache.find(role => role.name === roleName);
  ...
  if(command === "getgwawinners"){
    ...
    let gwa = message.channel.messages.fetch(args[0])
  }
  ...
  if (command === "makemute") {
    if (message.author.id === config.ownerID) {
      try {
        let muterole = await message.guild.cache.createRole({
          name: "Muted",
          color: "#000001",
          permissions: []
        });
...
//   }
  ...
  if (command === "event" && message.channel.id === config.eventchan) {
    ...
    if (eventname.toLowerCase().includes("worship") || eventname.toLowerCase().includes("music"))
      locationping = "<@&695018262982950923>";
    client.guilds.cache
      .get(config.server)
      .channels.cache.get(config.alerts)
      .send(
...
//   }
  ...
  if (command === "newteam") {
    ...
    const newargs = message.content
      ...
      .split(/ +/g);
    const command = args.shift().toLowerCase();
    let bottomposition = message.guild.roles.cache.find(
    ...
    message.guild.channels.create(newargs.slice(1).join("-"), {
...
function questperm(userid) {
  ...
  let user = client.guilds.cache.get(config.server).members.cache.get(userid);
  if (userid === config.ownerID) return true;
  if (!user) return false;
  if (user.roles.cache.find(role => role.id === "666379154631163905")) return true; // quest creators
  return false;
}
...
function modperm(userid) {
  ...
  if (userid === config.ownerID) return true;
  if (!user) return false;
  if (user.roles.cache.find(role => role.id === "593938752863338507")) return true; // discord mods
  return false;
}
...
function manageperm(userid) {
  ...
  if (!user) return false;
  if (user.roles.cache.find(role => role.id === "666379154631163905")) return true; // quest creators
  if (user.roles.cache.find(role => role.id === "621046093777731616")) return true; // admins
  if (user.roles.cache.find(role => role.id === "593938752863338507")) return true; // discord mods
  if (user.roles.cache.find(role => role.id === "511359440860086272")) return true; // leaders
  if (user.roles.cache.find(role => role.id === "650836005007654952")) return true; // elders
  if (user.roles.cache.find(role => role.id === "650724428224921611")) return true; // volunteers
  return false;
}
...
function updatestats() {
  let vrc = client.guilds.cache.get(config.server);
  let recroomchannel = client.channels.cache.get(config.stats.recroom);
  let altspacevrchannel = client.channels.cache.get(config.stats.altspacevr);
  let vrchatchannel = client.channels.cache.get(config.stats.vrchat);
  let volunteerchannel = client.channels.cache.get(config.stats.volunteer);
  let onlinechannel = client.channels.cache.get(config.stats.online);
  let recroom = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "RecRoom")
  ).size;
  let vrchat = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "VRChat")
  ).size;
  let altspacevr = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "AltspaceVR")
  ).size;
  let volunteer = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "Church Volunteers")
  ).size;
  let elder = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "Church Elders")
  ).size;
  let leaders = vrc.members.cache.filter(member =>
    member.roles.cache.find(r => r.name === "Church Leaders")
  ).size;
  let online = [];
  vrc.members.cache.forEach(member => {
